...
export default function AddressStore({ getAddressStore, addresses }) {
  const dataFormStep = useSelector((state) => state.dataFormStep);
  const [regionSelected, setRegionSelected] = useState({
    value: dataFormStep.value?.region_selected?.value || null,
    label: dataFormStep.value?.region_selected?.label || null,
  });
  const [addressSelected, setAddressSelected] = useState({
    value: dataFormStep.value?.address_store?.value || null,
    label: dataFormStep.value?.address_store?.label || null,
  });
        ...
        <Grid container spacing={3} justifyContent={"center"}>
          <Grid item xs={12} md={4}>
            ...
            <UISelect
              ...
              defaultValue={dataFormStep.value?.region_selected}
            />
          </Grid>
          <Grid item xs={12} md={4}>
            <UIBox mb={1} ml={0.5} lineHeight={0} display="inline-block">
